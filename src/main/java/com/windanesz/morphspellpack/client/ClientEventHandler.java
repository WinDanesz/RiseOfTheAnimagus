...
public class ClientEventHandler {
	...
	public static void RenderGameOverlayEvent(RenderGameOverlayEvent.Post event) {
		...
		if (player != null && player.isSpectator()) {
			return; // Spectators shouldn't have the spell HUD!
		}
...
//				y = top + (player.world.rand.nextInt(3) - 1);
//			}

			drawTexturedModalRect(x, y, 16 + background * 9, 27, 9, 9);

			if (idx < level) { drawTexturedModalRect(x, y, icon + 36, 27, 9, 9); } else if (idx == level) { drawTexturedModalRect(x, y, icon + 45, 27, 9, 9); }
		}
